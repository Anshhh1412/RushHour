...
process_r = [ Implies(Not(i == red[0]), r[m][i][j]==False) for m in range(1,k+1) for i in range(n) for j in range(n)]

# movement restriction(vertical)

move_v = [ Implies(v[m][i][j], eo( 
			v[m+1][i][j],
...
move_v_2n3_0 = [ Implies(v[m][i][0], eo( 
					v[m+1][i][0],
...
move_v1_i_0 = [ Implies(v[m+1][i][0], eo( 
					v[m][i][0],
...
move_v_n2_0 = [Implies(v[m][n-2][0], Xor( 
				v[m+1][n-2][0],
...
move_h_0_2n3 = [ Implies(h[m][0][j], eo( 
					h[m+1][0][j],
...
move_h1_0_j = [ Implies(h[m+1][0][j], eo( 
					h[m][0][j],
...
move_h_0_n2 = [Implies(h[m][0][n-2], Xor( 
				h[m+1][0][n-2],
...
if red[0] != 0:
	...
	move_r1 = [ Implies(r[m+1][red[0]][j], eo( 
			r[m][red[0]][j],
	...
	move_r_red_n2 = [ Implies(r[m][red[0]][n-2], Xor( 
					r[m+1][red[0]][n-2],
	...
	move_r_0_0 = [ Implies(r[m][0][0], Xor( 
					r[m+1][0][0],
					And(r[m+1][0][1],Not(mns[0][2]),Not(h[m][0][2]),Not(v[m][0][2]) ),	
					))
					for m in range(k)]

	move_r1_0_0 = [ Implies(r[m+1][0][0], Xor( 
				r[m][0][0],
				And(r[m][0][1],Not(mns[0][0]),Not(v[m][0][0]) ),	
				))
		   	 	for m in range(k)]

	move_r_0_1 = [  Implies(r[m][0][1], eo( 
						r[m+1][0][1],
	...
	move_r_0_2n3 = [ Implies(r[m][0][j], eo( 
						r[m+1][0][j],
	...
	move_r1_0_j = [ Implies(r[m+1][0][j], eo( 
					r[m][0][j],
	...
	move_r_0_n2 = [Implies(r[m][0][n-2], Xor( 
					r[m+1][0][n-2],
